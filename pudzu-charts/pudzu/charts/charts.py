...
def generate_legend(
    ...
    if any(
        not isinstance(box, Image.Image) and non_string_sequence(label) and size[1] == ...
    ...
    if len(boxes) > 0:
        max_box_width = max(
            box.width if isinstance(box, Image.Image) else size[0] for box, size in zip(boxes, box_sizes)
        )
        ...
        if box_mask is not None:
            ...
            label_img = label_img.overlay(
                Image.new("RGBA", boxes_size, bg),
                (1, spacing),
                mask=box_mask.resize(boxes_size).invert_mask(),
            )
    else:
        label_img = None

    legend = Image.from_column(
        [i for i in [header, label_img, footer] if i is not None], padding=padding, xalign=0, bg=bg
    )

    if border:
        legend = legend.pad(2, bg).pad(1, fg)
    return legend
...
def bar_chart(
    ...
    grid_interval=None,
    grid_width=1,
    tick_interval=Ellipsis,
    label_interval=Ellipsis,
    label_font=None,
    ...
    if isinstance(clabels, BarChartLabelPosition):
        clabels = {clabels: ...}
    if isinstance(rlabels, BarChartLabelPosition):
        rlabels = {rlabels: ...}
    ...
    if type in [BarChartType.SIMPLE, BarChartType.OVERLAYED]:
        ...
        if not all(
            k
            in [
                BarChartLabelPosition.ABOVE,
                BarChartLabelPosition.BELOW,
                BarChartLabelPosition.OUTSIDE,
            ]
            for k in rlabel_dict.keys()
        ):
    ...
    def make_fn_arg(input):
        if non_string_iterable(input):
            return lambda *args: input[args[0] % len(input)]
        elif not callable(input):
            return lambda *args: input
        else:
            return ignoring_extra_args(input)
    ...
    def make_box(fill, size, horizontal=horizontal):
        if callable(fill):
            return hzimg(fill(hzsize(size, horizontal=horizontal)), horizontal=horizontal)
        elif isinstance(fill, Image.Image):
            return hzimg(fill.resize(hzsize(size, horizontal=horizontal)), horizontal=horizontal)
        else:
    ...
    for r, row in enumerate(data.values):
        ...
        for c, v in enumerate(row):
            ...
            if type in [BarChartType.STACKED, BarChartType.STACKED_PERCENTAGE]:
                ...
                if nbar.height > 0:
                    nbar = with_inside_label(nbar)
                else:
                    pbar = with_inside_label(pbar)
    ...
    if legend_position is not None:
        ...
        for c in range(len(data.columns)):
            fill = color_fn(c, 0, 0)
            boxes.append(
                make_box(fill, lsize_fn(c), False) if callable(fill) or isinstance(fill, Image.Image) else fill
            )
    ...
    if grid_width:
        griddraw.line(
            [(tick_size, y_coordinate_fn(ymin)), (tick_size, y_coordinate_fn(ymax))],
            fill=fg,
            width=grid_width,
        )
    if grid_interval is not None:
        for i in range(ceil(ymin / grid_interval), floor(ymax / grid_interval) + 1):
            ...
            if grid_width:
                griddraw.line(
                    [(tick_size, y), (chart.width, y)],
                    fill=fg if i == 0 else gridcolor,
                    width=grid_width,
                )
    ...
    if label_interval is not None and ylabels is not None:
        for i in range(ceil(ymin / label_interval), floor(ymax / label_interval) + 1):
            ...
            if label is not None:
                chart = chart.pin(
                    hzimg(label),
                    (-tick_size - 10, y_coordinate_fn(y)),
                    align=(1, 0.5),
                    bg=bg,
                    offsets=offsets,
                )
    ...
    for clabels_pos, clabel_fn in clabel_dict.items():
        if clabels_pos == BarChartLabelPosition.INSIDE:
            continue
        for r, row in enumerate(data.values):
            for c, v in enumerate(row):
                ...
                if isinstance(label, str):
                    label = (
                        Image.from_text(label, label_font, fg=fg, align="center", padding=hzsize((0, 2)))
                        if label_font
                        else None
                    )
                if label is None:
    ...
    for rlabels_pos, rlabel_fn in rlabel_dict.items():
        for r, row in enumerate(data.values):
            ...
            if type in [
                ...
                BarChartType.STACKED_PERCENTAGE,
                BarChartType.OVERLAYED,
            ]:
            ...
            if rlabels_pos == BarChartLabelPosition.ABOVE:
                ...
                chart = chart.pin(
                    hzimg(label),
                    (x, chart.height - offsets.y + old_offsets.d),
                    align=(0.5, 0),
                    bg=bg,
                    offsets=offsets,
                )
    ...
    if xlabel is not None:
        chart = chart.pin(
            hzimg(xlabel),
            (int((chart.width - offsets.x) / 2), chart.height - offsets.u),
            align=(0.5, 0),
            bg=bg,
            offsets=offsets,
        )
    if ylabel is not None:
        chart = chart.pin(
            hzimg(ylabel),
            (-offsets.l, int((chart.height - offsets.y) / 2)),
            align=(1, 0.5),
            bg=bg,
            offsets=offsets,
        )
...
def time_chart(
    ...
    timeline_height,
    interval_data=None,
    interval_start_key="start",
    interval_end_key="end",
    interval_color_key="color",
    interval_label_key=None,
    event_data=None,
    event_time_key="time",
    event_image_key=None,
    event_label_key=None,
    xmin=None,
    xmax=None,
    fg="white",
    bg="black",
    ...
    if xmin is None:
        xmin = min(
            fn(d)
            for fn, data in ((interval_start_fn, interval_data), (event_time_fn, event_data))
            for df in data
    ...
    if xmax is None:
        xmax = max(
            fn(d)
            for fn, data in ((interval_end_fn, interval_data), (event_time_fn, event_data))
            for df in data
            for _, d in df.iterrows()
        )
    if xmin >= xmax:
    ...
    for intervals, events, llabel, rlabel in zip_longest(interval_data, event_data, labels_left, labels_right):
        ...
        if intervals is not None:
            ...
            for _, d in intervals.iterrows():
                ...
                bar = (
                    Image.from_pattern(color, (w, h))
                    if isinstance(color, Image.Image)
                    else Image.new("RGBA", (w, h), color)
                )
                ...
                for pos, label_fn in interval_labels_dict.items():
                    ...
                    if TimeChartLabelPosition.INSIDE in pos and img.width < (bar.width - 3) and img.height < bar.height:
                        ...
                        if TimeChartLabelPosition.BELOW in pos:
                            nextpos[pos] = TimeChartLabelPosition.BELOW
                    elif TimeChartLabelPosition.BELOW in pos:
                        timeline = timeline.pin(
                            img,
                            ((start + end) // 2, timeline_height),
                            align=(0.5, 0),
                            bg=bg,
                            offsets=offsets,
                        )
                        if TimeChartLabelPosition.ABOVE in pos:
                            nextpos[pos] = TimeChartLabelPosition.ABOVE
            ...
            for _, d in events.iterrows():
                ...
                for pos, label_fn in event_labels_dict.items():
                    ...
                    if isinstance(img, str):
                        img = Image.from_text(img, label_font, fg=fg, padding=2)
                    if (
                        TimeChartLabelPosition.INSIDE in pos
                        and img.width < event_img.width
                        and img.height < event_img.height
                    ):
                        timeline = timeline.pin(img, (middle, timeline_height // 2), bg=bg, offsets=offsets)
                    elif TimeChartLabelPosition.ABOVE in pos and nextpos[pos] != TimeChartLabelPosition.BELOW:
                        timeline = timeline.pin(
                            img,
                            (middle, min(0, (timeline_height - event_img.height) // 2)),
                            align=(0.5, 1),
                            bg=bg,
                            offsets=offsets,
                        )
                        ...
                        timeline = timeline.pin(
                            ...
                            (
                                middle,
                                max(timeline_height, (timeline_height + event_img.height) // 2),
                            ),
                            align=(0.5, 0),
                            bg=bg,
                            offsets=offsets,
    ...
    if grid_interval is not None:
        ...
        while grid_val <= xmax:
            griddraw.line(
                [(xoffset + xvalue(grid_val), 0), (xoffset + xvalue(grid_val), chart.height)],
                fill=gridcolor,
            )
...
def grid_chart(
    ...
    yalign=0.5,
    padding=(0, 0, 0, 0),
    group_fg_colors=tuple(VegaPalette10),
    group_bg_colors=lambda _, c: c._replace(alpha=128),
    group_bg_patterns=None,
    group_border=2,
    group_padding=(0, 0, 0, 0),
    group_rounded=True,
    row_label=Ellipsis,
    col_label=Ellipsis,
    corner_label=None,
    ...
    if isinstance(col_label, GridChartLabelPosition):
        col_label = {col_label: ...}
    if isinstance(row_label, GridChartLabelPosition):
        row_label = {row_label: ...}
    ...
    img_widths = [
        max(img.width if img is not None else 0 for img in column) + padding.x for column in zip_longest(*img_array)
    ]

    group_array = [
        [remove_duplicates(make_sequence(group_fn(v, r, c))) for c, v in enumerate(row)]
        for r, row in enumerate(data.values)
    ]
    ...
    def group_cmp(group, *same, diff=()):
        return all(
            0 <= r < len(img_heights) and 0 <= c < len(img_widths) and group in group_array[r][c] for r, c in same
        ) and not any(
            0 <= r < len(img_heights) and 0 <= c < len(img_widths) and group in group_array[r][c] for r, c in diff
        )

    def OuterCorner(size, fg, bg, bcol, bwidth=group_border):
        if bwidth == 0:
            return Quadrant(size, fg, bg)
        if size[0] <= bwidth or size[1] <= bwidth:
            return Quadrant(size, bcol, bg)
        size2 = (size[0] - bwidth, size[1] - bwidth)
        bg_img = Quadrant(size, fg, bg)
        border_img = MaskIntersection(
            size,
            bcol,
            bcol._replace(alpha=0),
            masks=[Quadrant(size), Quadrant(size2).pad((bwidth, bwidth, 0, 0), 0).invert_mask()],
        )
        return bg_img.place(border_img)

    def InnerCorner(size, fg, bg, bcol, bwidth=group_border):
        if bwidth == 0:
            return Quadrant(size, fg, bg, invert=True)
        size2 = (size[0] + bwidth, size[1] + bwidth)
        ...
        border_img = MaskIntersection(
            ...
            masks=[
                Quadrant(size2),
                Quadrant(size, invert=True).pad((bwidth, bwidth, 0, 0), "white"),
            ],
        )
        return bg_img.place(border_img)
        ...
        for c, _ in enumerate(row):
            ...
            if groups:
                ...
                for g in group_array[r][c]:
                    ...
                    if group_rounded and group_cmp(g, diff=[(r - 1, c), (r, c + 1)]):
                        mid.paste(
                            OuterCorner((gp.l, gp.d), cbg, tbg, cfg).transpose(Image.FLIP_LEFT_RIGHT),
                            (mid.width - gp.l, 0),
                        )
                    if group_rounded and group_cmp(g, diff=[(r + 1, c), (r, c - 1)]):
                        mid.paste(
                            OuterCorner((gp.r, gp.u), cbg, tbg, cfg).transpose(Image.FLIP_TOP_BOTTOM),
                            (0, mid.height - gp.u),
                        )
                    if group_rounded and group_cmp(g, diff=[(r + 1, c), (r, c + 1)]):
                        mid.paste(
                            OuterCorner((gp.l, gp.u), cbg, tbg, cfg).transpose(Image.ROTATE_180),
                            (mid.width - gp.l, mid.height - gp.u),
                        )

                    top = Rectangle((img_widths[c] - gp.x, gp.u), cbg if group_cmp(g, (r - 1, c)) else tbg)
                    if group_border and (
                        group_cmp(g, (r - 1, c), diff=[(r, c - 1)]) or group_cmp(g, (r - 1, c), diff=[(r - 1, c - 1)])
                    ):
                    ...
                    if group_border and (
                        group_cmp(g, (r, c - 1), diff=[(r - 1, c)]) or group_cmp(g, (r, c - 1), diff=[(r - 1, c - 1)])
                    ):
                    ...
                    top_left = Rectangle(
                        (gp.l, gp.u),
                        cbg if group_cmp(g, (r - 1, c), (r, c - 1), (r - 1, c - 1)) else tbg,
                    )
                    top_right = Rectangle(
                        (gp.r, gp.u),
                        cbg if group_cmp(g, (r - 1, c), (r, c + 1), (r - 1, c + 1)) else tbg,
                    ...
                    if group_rounded and group_cmp(g, (r - 1, c), (r, c - 1), diff=[(r - 1, c - 1)]):
                        img.paste(
                            InnerCorner((gp.l, gp.u), cbg, tbg, cfg).transpose(Image.ROTATE_180),
                            (0, 0),
                        )
                    if group_rounded and group_cmp(g, (r - 1, c), (r, c + 1), diff=[(r - 1, c + 1)]):
                        img.paste(
                            InnerCorner((gp.r, gp.u), cbg, tbg, cfg).transpose(Image.FLIP_TOP_BOTTOM),
                            (img.width - gp.r - group_border, 0),
                        )
                    if group_rounded and group_cmp(g, (r + 1, c), (r, c - 1), diff=[(r + 1, c - 1)]):
                        img.paste(
                            InnerCorner((gp.l, gp.d), cbg, tbg, cfg).transpose(Image.FLIP_LEFT_RIGHT),
                            (0, img.height - gp.d - group_border),
                        )
                    if group_rounded and group_cmp(g, (r + 1, c), (r, c + 1), diff=[(r + 1, c + 1)]):
                        img.paste(
                            InnerCorner((gp.r, gp.d), cbg, tbg, cfg),
                            (img.width - gp.r - group_border, img.height - gp.d - group_border),
                        )
    ...
    for rlabel_pos, rlabel_fn in rlabel_dict.items():
        ...
        for r, row in enumerate(data.values):
            ...
            if label is not None:
                label = Image.new(
                    "RGBA",
                    (label.width + padding.x, max(label.height + padding.y, img_heights[r])),
                    tbg,
                ).place(label, align=(xalign[row_n], yalign[1]))
            if groups:
    ...
    for clabel_pos, clabel_fn in clabel_dict.items():
        ...
        for c, col in enumerate(data.values.transpose()):
            ...
            if label is not None:
                label = Image.new(
                    "RGBA",
                    (max(label.width + padding.x, img_widths[c]), label.height + padding.y),
                    tbg,
                ).place(label, align=(xalign[1], yalign[col_n]))
    ...
    if GridChartLabelPosition.TOP in clabel_dict:
        yaligns[0] = yalign[0]
    if GridChartLabelPosition.BOTTOM in clabel_dict:
        yaligns[-1] = yalign[-1]
    ...
    if groups:
        ...
        if group_bg_patterns:
            ...
            group_patterns = {
                euler_bgs[groups]: group_patterns
                for groups in group_combos
                if groups
                for group_colors in [[RGBA(group_bg_col_fn(g)) for g in groups]]
                for group_patterns in [ignoring_extra_args(group_bg_patterns)(groups, group_colors)]
...
def name_csv_path(map):
    return splitext(map)[0] + ".csv"


def boundingbox_csv_path(map):
    return splitext(map)[0] + "_bbox.csv"


def labelbox_img_path(map):
    return splitext(map)[0] + "_lbox" + splitext(map)[1]


def labelbox_csv_path(map):
    return splitext(map)[0] + "_lbox.csv"


def overlay_img_path(map):
    return splitext(map)[0] + "_ov" + splitext(map)[1]


def overlay_mask_img_path(map):
    return splitext(map)[0] + "_ovmask" + splitext(map)[1]
...
def load_name_csv(map):
    return expand_color_columns(pd.read_csv(name_csv_path(map)), "color")


def load_boundingbox_csv(map):
    return expand_color_columns(pd.read_csv(boundingbox_csv_path(map)), "bbox", "color")


def load_labelbox_csv(map):
    return expand_color_columns(pd.read_csv(labelbox_csv_path(map)), "bbox", "color")
...
def generate_name_csv(map, presorted=(), sort=ImageMapSort.HORIZONTAL, overwrite=False):
    ...
    if sort == ImageMapSort.USAGE:
        ...
        data = np.array(img)
        if sort == ImageMapSort.HORIZONTAL:
    ...
    rs = [
        {"color": "|".join(str(x) for x in c), "name": "color{}".format(i), "label_align": ""}
...
def generate_bbox_csv(map, labels=True):
    ...
    rs = [
        {
            "color": "|".join(str(x) for x in c[:3]),
            "bbox": "|".join(str(x) for x in (xmin[c], ymin[c], xmax[c], ymax[c])),
        }
        for c in xmin
        if RGBA(c).alpha == 255
    ]
...
def map_chart(
    ...
    for c, name in colors:
        ...
        if not isinstance(color, Image.Image):
            ...
            pattern = color.resize((bbox.width, bbox.height)).pad(
                (bbox.l, bbox.u, img.width - bbox.r, img.height - bbox.d), 0
            )
...
def month_chart(
    ...
    day_overlay=None,
    day_start=0,
    out_of_month_bg="white",
    out_of_month_label=None,
    out_of_month_overlay=None,
    weekday_height=20,
    weekday_bg="#A0A0A0",
    weekday_label=lambda d: d.date_format("{W}")[:3].upper(),
    weekday_overlay=None,
    month_height=30,
    month_bg="#606060",
    month_label="{M} {Y}",
    month_overlay=None,
    ...
    # Arguments and defaults
    if month_height == Ellipsis:
        month_height = cell_height
    if weekday_height == Ellipsis:
        weekday_height = cell_height
    if out_of_month_bg == Ellipsis:
        out_of_month_bg = day_bg
    if out_of_month_label == Ellipsis:
        out_of_month_label = day_label
    if out_of_month_overlay == Ellipsis:
        out_of_month_overlay = day_overlay

    if callable(fonts):
        fonts = [fonts(bold=True), fonts(italics=True), fonts()]
    elif isinstance(fonts, ImageFont.FreeTypeFont):
        fonts = [fonts] * 3
    padding = (cell_padding, cell_padding, 0, 0)

    def make_fn(overlay):
        return ignoring_extra_args(overlay if callable(overlay) else (lambda d: overlay))
    ...
    if isinstance(day_start, str):
        day_start = month.calendar.WEEKDAYS.index(day_start)
    ...
    month_bg, month_label, month_overlay = (
        month_bg_fn(*fnargs),
        month_label_fn(*fnargs),
        month_overlay_fn(*fnargs),
    )
    month_img = (
        Image.from_pattern(month_bg, (month_width, month_height))
        if isinstance(month_bg, Image.Image)
        else Image.new("RGBA", (month_width, month_height), month_bg)
    )
    if isinstance(month_label, str):
        month_label = Image.from_text(month_label, fonts[0], fg)
    if month_label:
        month_img = month_img.place(month_label)
    if month_overlay:
        month_img = month_img.place(month_overlay)
    ...
    if weekday_height:
        ...
        for day in itertools.islice(fullrange, week_length):
            ...
            weekday_bg, weekday_label, weekday_overlay = (
                ...
                weekday_label_fn(*fnargs),
                weekday_overlay_fn(*fnargs),
            )
            weekday_img = (
                Image.from_pattern(weekday_bg, (cell_width, weekday_height))
                if isinstance(weekday_bg, Image.Image)
                else Image.new("RGBA", (cell_width, weekday_height), weekday_bg)
            )
            if isinstance(weekday_label, str):
                weekday_label = Image.from_text(weekday_label, fonts[1], fg)
            if weekday_label:
                weekday_img = weekday_img.place(weekday_label)
            if weekday_overlay:
                weekday_img = weekday_img.place(weekday_overlay)
    ...
    for week in generate_batches(fullrange, week_length):
        ...
        for day in week:
            if day in month:
                bg_fn, label_fn, overlay_fn = day_bg_fn, day_label_fn, day_overlay_fn
            else:
                bg_fn, label_fn, overlay_fn = (
                    out_of_month_bg_fn,
                    out_of_month_label_fn,
                    out_of_month_overlay_fn,
                )
            fnargs = (day, cell_width, cell_height)
            bg, label, overlay = bg_fn(*fnargs), label_fn(*fnargs), overlay_fn(*fnargs)
            day_img = (
                Image.from_pattern(bg, (cell_width, cell_height))
                if isinstance(bg, Image.Image)
                else Image.new("RGBA", (cell_width, cell_height), bg)
            )
            if isinstance(label, str):
                label = Image.from_text(label, fonts[2], fg)
            if label:
                day_img = day_img.place(label)
            if overlay:
                day_img = day_img.place(overlay)
    ...
    month_image = (
        month_image.resize_fixed_aspect(width=month_width).pad(padding, fg) if month_image else Image.EMPTY_IMAGE
    )
