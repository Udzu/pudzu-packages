...
class Calendar(object):
    ...
    def date_format(self, ymd, weekday, format_string="{W}, {D} {M} {Y}"):
        ...
        return format_string.format(
            W=self.weekday_name(weekday),
            D=self.day_name(ymd),
            M=self.month_name(ymd),
            Y=self.year_name(ymd),
            N=self.date_name(ymd),
            d=ymd.day,
            m=ymd.month,
            y=ymd.year,
        )
    ...
    def days_in_month(self, year, month):
        if month == 2:
            return (
                28
                + int(year % 4 == 0)
                - int(self.gregorian and year % 100 == 0 and year % 400 != 0)
            )
...
class HebrewCalendar(Calendar):
    ...
    def month_name(self, ymd):
        return self.MONTHS[
            ymd.month - 1 + int(ymd.month >= 12 and self.months_in_year(ymd.year) == 13)
        ]
...
isoweek = ISOCalendar()
gregorian = DerivedCalendar(
    WesternCalendar(),
    _era_to_astronomical,
    _astronomical_to_era,
    "Gregorian",
    year_name=_era_year_name,
)
julian = DerivedCalendar(
    WesternCalendar(julian=True),
    _era_to_astronomical,
    _astronomical_to_era,
    "Julian",
    year_name=_era_year_name,
)
islamic = DerivedCalendar(
    IslamicCalendar(),
    _era_to_astronomical,
    _astronomical_to_era,
    year_name=partial(_era_year_name, ad=" AH", bc=" BH"),
)
hebrew = DerivedCalendar(
    HebrewCalendar(),
    _civil_to_ecclesiastical,
    _ecclesiastical_to_civil,
    add_months=Calendar.add_months,
    add_years=_hebrew_add_years,
)
...
class Date(object):
    ...
    def __repr__(self):
        date_string = self.calendar.date_format(
            self.ymd, self.weekday, *([self.format] if self.format else [])
        )
...
class DateRange(collections.abc.Sequence):
    ...
    def __sub__(self, other):
        ...
        if non_string_sequence(difference, Date):
            return DateRange(
                difference[0], difference[1], calendar=self.calendar, format=self.format
            )
        else:
            return difference
...
class ApproximateDate(DateRange):
    ...
    def __init__(self, date, precision, circa=False, calendar=gregorian):
        ...
        if precision == DatePrecision.DAY:
            ...
            start = Date((year, month, 1), calendar=calendar) - delta
                ...
                Date((year, month, calendar.days_in_month(year, month)), calendar=calendar) + delta
            )
            ...
            start = Date((year, 1, 1), calendar=calendar) - delta
                ...
                Date(
                    (year, last_month, calendar.days_in_month(year, last_month)), calendar=calendar
                )
                + delta
            )
    ...
    def __repr__(self):
        ...
        elif self.precision in (
            DatePrecision.DECADE,
            DatePrecision.CENTURY,
            DatePrecision.MILLENNIUM,
        ):
            range = self.base_date.date_format("{Y}").replace(
